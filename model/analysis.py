"""Post-fit analysis utilities for ADAPEL.

Aggregates ``bootstrap`` + ``diagnostics`` + ``clinical``. Every function
takes a fitted ``ADAPEL`` instance (``model_self``) plus data and returns a
plain dict; ``ADAPEL`` wraps each as a thin method that adds
``_check_fitted()`` and feature selection.
"""
...
def compute_diagnostics(model_self, X: ArrayLike) -> dict:
    """Per-sample diagnostics for a fitted ADAPEL."""
    e = clip_e(
...
def estimate_e_value(
    ...
    """E-Value (VanderWeele & Ding 2017) for unmeasured confounding."""
    ate = model_self.estimate_ate(X)
    if outcome_type == "binary":
        p0 = float(np.clip(np.mean(model_self._m0_full.predict(X)), 1e-5, 1 - 1e-5))
        p1 = float(np.clip(np.mean(model_self._m1_full.predict(X)), 1e-5, 1 - 1e-5))
        rr = max(p1 / p0, p0 / p1)
    else:
...
def explain_surrogate(
    ...
    return "\n".join(
        line for line in export_text(surrogate, feature_names=names).split("\n")
        if line.strip()
    )
...
def fit_bootstrap(
    ...
    from .adapel import ADAPEL  # local import avoids cycle
